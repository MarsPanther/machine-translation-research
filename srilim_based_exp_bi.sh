...
#"$HOME/mosesdecoder/tools/irstlm-5.80.08/trunk/bin/add-start-end.sh" < "$LM/combined."$LANG2 >  "$LM/combined_lm."$LANG2 

#Using SRILM to compute ngram lang model
"$HOME/mosesdecoder/tools/srilm-1.7.2/bin/i686-m64/ngram-count" -text "$LM/combined_lm."$LANG2 -order 3 -lm "$LM/lm.$LANG1-$LANG2.arpa.$LANG2" -kndiscount1 -kndiscount2 -kndiscount3 -interpolate2 -interpolate3 -gt3min 1 -gt2min 1

####Binarize LM-------------------------------------------------------------------------------------

#"$HOME/mosesdecoder/bin/build_binary" "$LM/lm.$LANG1-$LANG2.arpa.$LANG2" "$LM/lm.$LANG1-$LANG2.blm.$LANG2"
...
-f $LANG1 -e $LANG2 \
-lm 0:3:"$LM/lm.$LANG1-$LANG2.arpa.$LANG2" \
...
-in "$WORKING/train/model/model/phrase-table.gz" \
-nscores 4 \
...
"$HOME/mosesdecoder/scripts/training/filter-model-given-input.pl" "$WORKING/filtered-test."$LANG1 "$WORKING/mert-work/moses.ini" "$CORPUS/test.true."$LANG1 -Binarizer "$HOME/mosesdecoder/bin/processPhraseTableMin"
